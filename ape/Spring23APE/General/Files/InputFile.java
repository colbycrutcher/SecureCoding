...
//startMethod
	public static boolean openInputFile(String inputFileName){
		if (inputFileName == null)
			throw new IllegalArgumentException("file name passed to openInputFile was null");
		if (inputFileName.equals(""))
			throw new IllegalArgumentException("file name passed to openInputFile was empty");
		if (isOpen){
			errorMsg = "Scanner already open and attached to: " + file.getName();
			return false;
		}
		
		file = new File(inputFileName);
		if (!file.exists()){
			//System.out.println(inputFileName + " not there");
			errorMsg = "file name passed to openInputFile: " + inputFileName + ", does not exist";
			return false;
		}
		try{
